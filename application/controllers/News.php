...
class News extends MY_Controller
{
	...
	function view()
	{
		...
		$catalogurl = $this->catalognew_model->get_info($post->catalog_id)->url;
		if (!$post) {
			redirect();
		}
		...
		// 	$input['limit'] = [5, 0];
		// 	$input['order'] = check_sort($this->setPage->sort_news, 'news');
		//  $input['order1'] = ['news.id', 'desc'];
		...
		$this->mybreadcrumb->add('Trang chủ', base_url());
		$this->mybreadcrumb->add($catalog->name, base_url($catalog->url));
		$this->mybreadcrumb->add($post->name, base_url($catalog->url . '/' . $post->url));
		$this->data['breadcrumb'] = $this->mybreadcrumb->render();
		// View
	...
	public function tags()
	{
		...
		$input['join'] = ['tags_news', 'tags_news.new_id = news.id'];
		$input['join1'] = ['catalog_new', 'catalog_new.id = news.catalog_id'];
		...
		if (count($this->catalognew_model->menucon($catalog->id)) > 0) {
			...
			if ($catalog_subs) {
				...
				foreach ($catalog_subs as $sub_id) {
					...
					$input['join'] = ['news_catalognew', 'news_catalognew.new_id = news.id'];
					$input['where'] = ['status' => 1, 'timer <=' => now()];
					$input['where_in'] = ['news_catalognew.catalog_id', $sub_child];
					$input['group_by'] = 'news.id';
					$input['order'] = check_sort($setPage->sort_news, 'news');
					$input['order1'] = ['news.id', 'desc'];
	...
	{ // Hiển thị 1 layout
		...
		$input['order1'] = ['news.id', 'desc'];
		// Pagination
		$config = $this->frontpagination->config($this->news_model->get_total($input), base_url($catalog->url), $setPage->pagination_catalog_new, $_GET, base_url($catalog->url), 2);
		$this->pagination->initialize($config);
