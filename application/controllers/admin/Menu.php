...
class Menu extends My_Controller
{
	...
	function __construct()
	{
		...
		$input['order'] = check_sort($this->setadmin->sort_catalog_new);
		$this->catalognew = $this->catalognew_model->get_list($input);
		$this->data['catalognew'] = $this->catalognew;
		//Load bài viết
		$input = [];
		$input['order'] = check_sort($this->setadmin->sort_news);
		...
		$input = [];
		$input['where'] = ['parent_id' => 0];
		$input['order'] = check_sort($this->setadmin->sort_catalog_service);
		$this->catalogservice = $this->catalogservice_model->get_list($input);
		$this->data['catalogservice'] = $this->catalogservice;
		//Load dịch vụ
		$input = [];
		$input['order'] = check_sort($this->setadmin->sort_services);
