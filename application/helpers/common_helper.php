...
function re_menu($url = '', $type, $id_type)
{
    if ($type == 'news' || $type == 'services' || $type == 'products' || $type == 'images' || $type == 'pages') {
        ...
        if ($type == 'news') { // New
            ...
            $post = $CI->news_model->get_info($id_type);
            if ($post) {
                $catalog = $CI->catalognew_model->get_info($post->catalog_id);
                if ($catalog) {
                    $url = $catalog->url . '/' . $url;
                }
            }
    ...
    if ($type == 'default') {
        ...
        if ($price > 0) {
            if ($discount > 0 && $discount < $price) {
                $html .= ' <strong class="text-danger">' . number_format($price) . '<sup>' . _unit . '</sup></strong>';
                $html .= '<span class="text-xs itext-9 ml-10"><s>' . number_format($discount) . '<sup>' . _unit . '</s></span>';
            } else {
                $html .= '<strong class="text-danger">' . number_format($price) . '<sup>' . _unit . '</sup></strong>';
            }
        } else {
            $html .= '<strong class="text-danger">Liên hệ</strong>';
        }
        $html .= '</div>';
    } else if ($type == 'detail') {
...
function newSidebar()
{
    ...
    $input = [];
    $input['select'] = 'name, url, image_link, catalog_id, timer';
    $input['where'] = ['status' => 1, 'sidebar' => 1, 'timer <=' => now()];
    $input['order'] = check_sort($setPage->sort_news);
