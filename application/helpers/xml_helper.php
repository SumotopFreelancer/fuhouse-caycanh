...
function processNews($CI, $row, $dom, $root)
{
    $result = $dom->createElement('url');
    $root->appendChild($result);
    $urlCate = $CI->catalognew_model->get_info($row->catalog_id)->url;
...
function processCatalog($CI, $row, $dom, $root)
{
    ...
    if ($total_rows > 0) {
        ...
        for ($i = 1; $i <= $page; $i++) {
            $result = $dom->createElement('url');
            $root->appendChild($result);
            $urlCate = base_url(_csp . '/' . $row->url);
            if ($i > 1) {
                $urlCate = base_url(_csp . '/' . $row->url . '/' . $i);
            }
            $result->appendChild($dom->createElement('loc', $urlCate));
            $result->appendChild($dom->createElement('lastmod', gmdate(DATE_W3C, $row->updated)));
            $result->appendChild($dom->createElement('priority', '0.8'));
        ...
        $result = $dom->createElement('url');
        $root->appendChild($result);
        $result->appendChild($dom->createElement('loc', base_url(_csp . '/' . $row->url)));
        $result->appendChild($dom->createElement('lastmod', gmdate(DATE_W3C, $row->updated)));
        $result->appendChild($dom->createElement('priority', '0.8'));
    }
}
...
function processProducts($CI, $row, $dom, $root)
{
    $result = $dom->createElement('url');
    $root->appendChild($result);
    $result->appendChild($dom->createElement('loc', base_url(_sp . '/' . $row->url)));
    $result->appendChild($dom->createElement('lastmod', gmdate(DATE_W3C, $row->updated)));
    $result->appendChild($dom->createElement('priority', '0.8'));
}
// Tagsproduct
...
function processCatalogService($CI, $row, $dom, $root)
{
    ...
    $input['join'] = ['services_catalogservice', 'services_catalogservice.service_id = services.id'];
    $input['where'] = ['status' => 1, 'timer <=' => now()];
...
function processCatalogImage($CI, $row, $dom, $root)
{
    ...
    $input['join'] = ['images_catalogimage', 'images_catalogimage.image_id = images.id'];
    $input['where'] = ['status' => 1, 'timer <=' => now()];
