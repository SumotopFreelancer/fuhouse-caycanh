...
class MY_Model extends CI_Model
{
	...
	protected function get_list_set_input($input = [])
	{
		...
		if ((isset($input['join1'])) && $input['join1']) {
			$this->db->join($input['join1'][0], $input['join1'][1]);
		}
		...
		if (isset($input['order'][0]) && isset($input['order'][1])) {
			...
			if (isset($input['order1'][0]) && isset($input['order1'][1])) {
				$this->db->order_by($input['order1'][0], $input['order1'][1]);
			}
		} else {
			$order = ($this->order == '') ? [$this->table . '.' . $this->key, 'desc'] : $this->order;
